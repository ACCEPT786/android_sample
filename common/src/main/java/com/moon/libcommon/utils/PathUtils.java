...
/**
 * Created by Ben on 2015/3/12.
 */
public class PathUtils {
    public final static String SD_PLUGIN_STORAGE_PATH = "TimoDL/";
    public final static String ROM_PLUGIN_STORAGE_PATH = "TimoDL";
    ...
    public void initDirs(Context context) {
        ...
        synchronized (rootPath) {
            ...
            if (enoughSpaceSizeSD()) {
                pluginStoragePath = Environment.getExternalStorageDirectory() + rootPath;
            } else {
                pluginStoragePath = context.getDir(rootPath, Context.MODE_PRIVATE).getAbsolutePath();
            }
    ...
    public String getPluginFilePath(Context context) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            pluginStoragePath = Environment.getExternalStorageDirectory() + "/" + SD_PLUGIN_STORAGE_PATH;
        } else {
            pluginStoragePath = context.getDir(ROM_PLUGIN_STORAGE_PATH, Context.MODE_PRIVATE).getAbsolutePath() + "/";
        }
        File file = new File(pluginStoragePath);
        if (!file.exists()) {
            file.mkdirs();
        }
    ...
    public static String createExamPath(Context context) {
        String time = DateUtils.getTimeOfFormat(System.currentTimeMillis(), "yyyy-MM-dd-HHmmss");
        String filename = context.getString(R.string.errorbook, time);
        return filename;
    }
